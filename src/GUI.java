	    	...
	    	public void printErr(){
	    	   print = false;	
	            ...
	            errField = new  JTextField();
	             errField.setText("File C:/SEBN_SHIPPING_LABEL/Settings.ini not found!");
...
//Inicializacia na faila s paketaja	 
 ...
 //izvli4ane na path kam statistikata                                                   /
	...
	//Izvli4ane na sleep time za printirane
		   	...
		   	String sleepTime = arrIni.substring(startLength4+3,endLength4).trim();           
		 ...
		 //izvli4ane na promenliva za pozicia na ekrana y
...
//Izvli4ane na daljina na PCS:  format na broikite
		   	...
		   	String lengthPCS = arrIni.substring(startLength8+3,endLength8).trim();
       ...
       public void actionPerformed(ActionEvent arg0) {
    	   ...
    	   try {
			getDoplani = fieldDoplni.getText().trim();	
			   ...
			   dopalniSample= getDoplani.substring(0,(getDoplani.length()-daljinaBroi-1));
			   
			   dopalniBroi= Integer.valueOf(getDoplani.substring(getDoplani.length()-daljinaBroi-1,getDoplani.length()));
			   ...
			   //obhojdane na masiva za sravnenie na dopalni texta s paketaja
			    	...
			    	for(int count = 0;count<=paketajArr.length-1;count++){
			    		...
			    		String temp = paketajArr[count].substring(startPaketaj,endPaketaj).trim();
			    		
			    		
			    		//System.out.println(	temp);
			    		if(temp.equals(dopalniSample)){
			    		  getMatchCode  = paketajArr[count].substring(0, paketajArr[count].length()-26).trim();  
